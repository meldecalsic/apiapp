...
async function getFloracioIA(lat, lng, altitud) {
  try {
    const mes = MONTHS[new Date().getMonth()];
    const res = await fetch("/api/analyze", {
      method:"POST", headers:{"Content-Type":"application/json"},
      body: JSON.stringify({ prompt_only:true, prompt:
        "Ets un expert apicultor de Catalunya. Dona en 1 sola línia (màx 60 caràcters) quines flors estan en floració ara per a un apiari a lat="+lat+", lng="+lng+", altitud="+altitud+"m, mes de "+mes+". Només el nom de les plantes, res més." })
    });
...
function MeteoWidget({ lat, lng, altitud }) {
  const [meteo,setMeteo]=useState(null); const [floracio,setFloracio]=useState(null); const [floLoad,setFloLoad]=useState(false);
  ...
  const demanarFloracio=async()=>{ setFloLoad(true); const f=await getFloracioIA(lat,lng,altitud||0); setFloracio(f); setFloLoad(false); };
  if (!lat||!lng) return null;
  return (
    <div style={{marginTop:12}}>
      ...
      <div style={{marginTop:8,padding:"6px 10px",background:"rgba(80,140,60,0.1)",borderRadius:6,border:"1px solid rgba(80,140,60,0.2)",fontSize:12,display:"flex",justifyContent:"space-between",alignItems:"center"}}>
        <span><span style={{color:"#7ac87a"}}>Floracio: </span><span style={{color:"#a8d8a8"}}>{floracio||"—"}</span></span>
        <button onClick={demanarFloracio} disabled={floLoad} style={{...S.btnGhost,padding:"3px 8px",fontSize:10}}>{floLoad?"...":"Actualitzar IA"}</button>
      </div>
    </div>
  );
}
  ...
  return (
    <div style={{position:"fixed",inset:0,background:"#000",zIndex:9999,overflow:"hidden"}} onTouchStart={onTouchStart} onTouchMove={onTouchMove} onTouchEnd={onTouchEnd}>
      <div style={{position:"absolute",top:12,right:12,display:"flex",gap:8,zIndex:10000}}>
        {scale>1.05&&<button onClick={reset} style={{background:"rgba(255,255,255,0.2)",border:"none",borderRadius:6,color:"#fff",padding:"6px 12px",fontSize:13,cursor:"pointer"}}>Reset</button>}
...
// ─── EMPTY REVIEW ─────────────────────────────────────────────────────────────
const emptyReview = () => ({ date:new Date().toISOString().split("T")[0], forcaColonia:null, quadresMel:null, pollen:null, quadresAbelles:null, quadresCria:null, criaEstat:null, estatReina:null, cellesReials:null, anyReina:null, varroaMes:null, varroaDia:null, varroaPct:null, tractamentMes:null, tipusTractament:null, quadresBuits:null, agressivitat:null, notes:"" });

// ─── REVIEW FORM ──────────────────────────────────────────────────────────────
function ReviewForm({ arnaNumero, initial, onSave, onCancel, loading, compact, photoUrl }) {
  const [f,setF]=useState(initial||emptyReview()); const set=(k,v)=>setF(p=>({...p,[k]:v}));
        ...
        <div style={{background:"rgba(255,50,50,0.05)",border:"1px solid rgba(255,80,80,0.15)",borderRadius:10,padding:12,marginBottom:12}}>
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"0 8px"}}>
            <div style={{marginBottom:12}}>
              ...
              <select value={f.varroaMes!=null?f.varroaMes:""} onChange={e=>set("varroaMes",e.target.value===""?null:+e.target.value)} style={{...S.input,background:"#100000",color:f.varroaMes==null?"#5a4a2a":"#fff"}}>
                <option value="">— Sense xinxeta —</option>
...
function ExplotacionsScreen({ usuari, onSelect, onLogout }) {
  ...
  const [mode,setMode]=useState(null); // "crear" | "unirse"
    ...
    <div style={{...S.page,padding:"0 0 40px"}}>
      <div style={{maxWidth:600,margin:"0 auto",padding:"0 16px"}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"18px 0 14px",borderBottom:"1px solid rgba(255,200,50,0.08)"}}>
          <div>
            <h1 style={{color:"#f5c842",fontSize:20,margin:0}}>🍯 CuidadorAbelles AI</h1>
            <p style={{color:"#7a6040",fontSize:11,margin:"2px 0 0"}}>{usuari.email}</p>
          </div>
        ...
        {mode==="crear"&&(
          <div style={{...S.card,marginBottom:16}}>
            ...
            <div style={{display:"flex",flexDirection:"column",gap:10}}>
              <div><label style={S.label}>Nom de l'explotació</label><input value={nom} onChange={e=>setNom(e.target.value)} style={S.input} placeholder="Cal Mel de la Muntanya"/></div>
              <div><label style={S.label}>REGA (codi oficial)</label><input value={rega} onChange={e=>setRega(e.target.value.toUpperCase())} style={S.input} placeholder="ES-08-12345"/></div>
              ...
              <div style={{display:"flex",gap:8}}>
                <button onClick={crear} disabled={saving} style={{...S.btnPri,flex:1}}>{saving?"Creant...":"Crear explotació"}</button>
        ...
        {mode==="unirse"&&(
          <div style={{...S.card,marginBottom:16}}>
            ...
            <div style={{display:"flex",flexDirection:"column",gap:10}}>
              ...
              <div><label style={S.label}>Contrasenya</label><input type="password" value={pwd} onChange={e=>setPwd(e.target.value)} style={S.input} placeholder="Contrasenya de l'explotació"/></div>
              {err&&<div style={{color:"#ff8888",fontSize:13}}>{err}</div>}
              <div style={{display:"flex",gap:8}}>
        ...
        {loading?<div style={{color:"#7a6040",textAlign:"center",padding:20}}>Carregant...</div>:(
          <div style={{display:"flex",flexDirection:"column",gap:10}}>
            ...
            {explotacions.length===0&&<div style={{textAlign:"center",padding:30,color:"#5a4a2a"}}>No pertanys a cap explotació. Crea'n una o uneix-te a una existent.</div>}
          </div>
        )}
      </div>
    </div>
...
function BulkProcessor({ apiariArnes, onComplete, onClose }) {
      ...
      <div style={{maxWidth:700,margin:"0 auto"}}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,margin:"20px 0 16px"}}>
          <div style={{position:"relative",...S.btnBlue,padding:"18px 12px",textAlign:"center",display:"flex",flexDirection:"column",alignItems:"center",gap:6,borderRadius:8,overflow:"hidden"}}>
            ...
            <input type="file" accept="image/*" capture="environment" onChange={e=>{if(e.target.files?.length)addFiles(e.target.files);e.target.value="";}} style={{position:"absolute",inset:0,opacity:0,cursor:"pointer",fontSize:0}}/>
          </div>
          <div style={{position:"relative",...S.btnGold,padding:"18px 12px",textAlign:"center",display:"flex",flexDirection:"column",alignItems:"center",gap:6,borderRadius:8,overflow:"hidden"}}>
            ...
            <input type="file" accept="image/*" multiple onChange={e=>{if(e.target.files?.length)addFiles(e.target.files);e.target.value="";}} style={{position:"absolute",inset:0,opacity:0,cursor:"pointer",fontSize:0}}/>
          </div>
        </div>
        {items.length>0&&(<div>
        ...
        {items.length===0&&<div style={{textAlign:"center",padding:"50px 20px",color:"#5a4a2a"}}><div style={{fontSize:52,marginBottom:12}}>📷</div><p style={{fontSize:14,color:"#7a6040"}}>Afegeix fotos de les fitxes per analitzar-les</p></div>}
      </div>
    </div>
  );
  ...
  if(phase==="processing") return (
    <div style={{...S.page,position:"fixed",inset:0,zIndex:1000,overflowY:"auto",padding:"0 14px 40px"}}>
      <div style={{maxWidth:700,margin:"0 auto"}}>
        <div style={{padding:"18px 0 14px",borderBottom:"1px solid rgba(255,200,50,0.08)"}}><h2 style={{margin:0,color:"#f5c842",fontSize:19}}>Analitzant fitxes...</h2><p style={{margin:"4px 0 0",color:"#7a6040",fontSize:11}}>Foto {Math.min(currentIdx+1,items.length)} de {items.length}</p></div>
...
function ArnaDetail({ arna, revisions, onAddRevision, onBack }) {
  const [tab,setTab]=useState("resum"); const [showForm,setShowForm]=useState(false);
  const [aiPrefill,setAiPrefill]=useState(null); const [aiLoad,setAiLoad]=useState(false);
  const [saveLoad,setSaveLoad]=useState(false); const [photoUrl,setPhotoUrl]=useState(null);

  const sorted=[...revisions].sort((a,b)=>a.date.localeCompare(b.date));
  const last=sorted[sorted.length-1];
  const chartData=sorted.map(r=>({d:r.date.slice(5),forca:r.forcaColonia,mel:r.quadresMel,abelles:r.quadresAbelles,cria:r.quadresCria,varroa:r.varroaPct}));

  const handlePhoto=async file=>{ if(!file) return; setAiLoad(true); setPhotoUrl(URL.createObjectURL(file)); const b64=await toBase64(file); const result=await readFitxaAI(b64,file.type); if(result){const{numero,...rest}=result;setAiPrefill({...emptyReview(),...rest});}else setAiPrefill(null); setShowForm(true); setAiLoad(false); };
  const handleSave=async data=>{ setSaveLoad(true); await onAddRevision({...data,arnaId:arna.id,id:"r_"+Date.now()}); setShowForm(false); setAiPrefill(null); setPhotoUrl(null); setSaveLoad(false); };
    ...
    <div style={S.page}>
      <div style={{maxWidth:700,margin:"0 auto",padding:"0 14px 40px"}}>
        <div style={{padding:"18px 0 14px",display:"flex",alignItems:"center",gap:10,borderBottom:"1px solid rgba(255,200,50,0.08)"}}>
          ...
          <div style={{flex:1}}><h2 style={{margin:0,color:"#f5c842",fontSize:20}}>Arna #{arna.numero}</h2><p style={{margin:0,color:"#7a6040",fontSize:11}}>{revisions.length} revisions</p></div>
        </div>
          ...
          {aiLoad?<span style={{color:"#f5c842",fontSize:13,alignSelf:"center"}}>Llegint fitxa amb IA...</span>:(
            <>
              <div style={{position:"relative",...S.btnBlue,display:"inline-flex",alignItems:"center",gap:6,overflow:"hidden"}}>
                📷 Fer foto
                <input type="file" accept="image/*" capture="environment" onChange={e=>{if(e.target.files?.[0])handlePhoto(e.target.files[0]);e.target.value="";}} style={{position:"absolute",inset:0,opacity:0,cursor:"pointer",fontSize:0}}/>
              </div>
              <button onClick={()=>{setAiPrefill(null);setPhotoUrl(null);setShowForm(true);}} style={S.btnGold}>✍️ Manual</button>
            </>
          )}
        </div>
        {showForm?(
          <div style={{...S.card,margin:"10px 0"}}>
            <ReviewForm arnaNumero={arna.numero} initial={aiPrefill} photoUrl={photoUrl} onSave={handleSave} onCancel={()=>{setShowForm(false);setPhotoUrl(null);}} loading={saveLoad}/>
          </div>
        ):(
          <>
            ...
            {tab==="resum"&&(<div>{last?(
              <div style={{display:"flex",flexDirection:"column",gap:12}}>
                ...
                <div style={S.card}>
                  ...
                  <div style={{marginTop:8,fontSize:12,color:"#aaa"}}>Cria: <span style={{color:"#fff"}}>{last.criaEstat||"—"}</span> · Cel·les Reials: <span style={{color:"#fff"}}>{last.cellesReials!=null?last.cellesReials:"—"}</span></div>
                </div>
                <div style={{...S.card,border:"1px solid rgba(255,100,100,0.15)",background:"rgba(255,50,50,0.02)"}}>
                  ...
                  <div style={{fontSize:13}}>Nivell Varroa: <span style={{fontWeight:700,color:"#fff"}}>{last.varroaPct!=null?["0-1%","2%","3%","Més de 3%"][last.varroaPct]:"—"}</span>{last.varroaMes!=null&&<span style={{fontSize:11,color:"#7a6040"}}> ({MONTHS[last.varroaMes]} {last.varroaDia||""})</span>}</div>
            ...
            {tab==="grafics"&&(<div style={{display:"flex",flexDirection:"column",gap:14}}>{chartData.length>1?(
              <>
                <div style={{...S.card,height:200}}><div style={{fontSize:11,color:"#d4a855",fontWeight:700,marginBottom:6}}>POBLACIÓ I CRIA</div><ResponsiveContainer width="100%" height="85%"><LineChart data={chartData}><CartesianGrid strokeDasharray="3 3" stroke="rgba(255,255,255,0.05)"/><XAxis dataKey="d" stroke="#6b5a3a" style={{fontSize:10}}/><YAxis stroke="#6b5a3a" style={{fontSize:10}}/><Tooltip contentStyle={{background:"#1a1200",border:"1px solid #f5c842",fontSize:11}}/><Line type="monotone" dataKey="abelles" stroke="#88bbff" name="Abelles" strokeWidth={2}/><Line type="monotone" dataKey="cria" stroke="#ff8888" name="Cria" strokeWidth={2}/><Line type="monotone" dataKey="mel" stroke="#f5c842" name="Mel" strokeWidth={1.5}/></LineChart></ResponsiveContainer></div>
                <div style={{...S.card,height:150}}><div style={{fontSize:11,color:"#ff8888",fontWeight:700,marginBottom:6}}>EVOLUCIÓ VARROA</div><ResponsiveContainer width="100%" height="80%"><LineChart data={chartData}><CartesianGrid strokeDasharray="3 3" stroke="rgba(255,255,255,0.05)"/><XAxis dataKey="d" stroke="#6b5a3a" style={{fontSize:10}}/><YAxis stroke="#6b5a3a" style={{fontSize:10}} domain={[0,3]} ticks={[0,1,2,3]}/><Tooltip contentStyle={{background:"#1a1200",border:"1px solid #ff8888",fontSize:11}}/><Line type="monotone" dataKey="varroa" stroke="#ff4444" name="Varroa" strokeWidth={2}/></LineChart></ResponsiveContainer></div>
              </>
            ...
            {tab==="historic"&&(<div style={{display:"flex",flexDirection:"column",gap:8}}>{[...sorted].reverse().map(r=>(
              <div key={r.id} style={{...S.card,padding:12,background:"rgba(255,255,255,0.02)"}}>
                ...
                <div style={{fontSize:12,color:"#ccc"}}>🍯 {r.quadresMel!=null?r.quadresMel:"—"}q · 🥚 {r.quadresCria!=null?r.quadresCria:"—"}q ({r.criaEstat||"—"}) · 🐝 {r.quadresAbelles!=null?r.quadresAbelles:"—"}q</div>
...
export default function App() {
  ...
  const handleAddRevision=async(revData)=>{
    const obj={arna_id:revData.arnaId,date:revData.date,forca_colonia:revData.forcaColonia,quadres_mel:revData.quadresMel,pollen:revData.pollen,quadres_abelles:revData.quadresAbelles,quadres_cria:revData.quadresCria,cria_estat:revData.criaEstat,estat_reina:revData.estatReina,celles_reials:revData.cellesReials,any_reina:revData.anyReina,varroa_mes:revData.varroaMes,varroa_dia:revData.varroaDia,varroa_pct:revData.varroaPct,tractament_mes:revData.tractamentMes,tipus_tractament:revData.tipusTractament,quadres_buits:revData.quadresBuits,agressivitat:revData.agressivitat,notes:revData.notes};
  ...
  // Normalitza revisions de Supabase al format intern
  const normRev=r=>({...r,forcaColonia:r.forca_colonia,quadresMel:r.quadres_mel,pollen:r.pollen,quadresAbelles:r.quadres_abelles,quadresCria:r.quadres_cria,criaEstat:r.cria_estat,estatReina:r.estat_reina,cellesReials:r.celles_reials,anyReina:r.any_reina,varroaMes:r.varroa_mes,varroaDia:r.varroa_dia,varroaPct:r.varroa_pct,tractamentMes:r.tractament_mes,tipusTractament:r.tipus_tractament,quadresBuits:r.quadres_buits,agressivitat:r.agressivitat,notes:r.notes});
  ...
  if(selArna) return (
    <ArnaDetail arna={selArna} revisions={revisions.filter(r=>r.arna_id===selArna.id).map(normRev)} onAddRevision={handleAddRevision} onBack={()=>setSelArna(null)}/>
  );
      ...
      <div style={{maxWidth:700,margin:"0 auto",padding:"20px 14px"}}>
        ...
        {!selApiari?(
                  ...
                  return (
                    <div key={ap.id} style={{...S.card,display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                      <div onClick={()=>setSelApiari(ap)} style={{cursor:"pointer",flex:1}}>
                        ...
                        <div style={{fontSize:12,color:"#7a6040",marginTop:2}}>{count} arnes · {ap.lat?`${ap.lat}, ${ap.lng}`:"Sense ubicació"}{ap.altitud?` · ${ap.altitud}m`:""}</div>
                      </div>
          ...
          <div>
                ...
                return (
                  <div key={ar.id} style={{...S.card,textAlign:"center",position:"relative",border:alerta?"1px solid rgba(255,140,40,0.5)":S.card.border}}>
                    ...
                    <div onClick={()=>setSelArna(ar)} style={{cursor:"pointer",paddingBottom:6}}>
                      ...
                      {last?(<><div style={{color:"#a0845c",fontSize:10,marginTop:3}}>{last.forcaColonia!=null?FORCE[last.forcaColonia]:"—"}</div><div style={{color:"#5a4a2a",fontSize:9}}>{last.date}</div></>):(<div style={{color:"#5a4a2a",fontSize:10,marginTop:4}}>Sense revisió</div>)}
                    </div>
            ...
            {apiariArnes.length===0&&<div style={{textAlign:"center",padding:40,color:"#5a4a2a"}}>No hi ha cap arna. Afegeix-ne una o escaneja una fitxa.</div>}
          </div>
        )}
      </div>
    </div>
